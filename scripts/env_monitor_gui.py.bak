...
class SerialMenuClient:
    ...
    def _send_cmd_with_string(self, cmd: int, value: str) -> None:
        """Send command followed by length-prefixed string"""
        ser = self._require_open()
        ser.write(bytes([cmd]))
    ...
    def _send_cmd_with_byte(self, cmd: int, value: int) -> None:
        """Send command followed by a single byte"""
        ser = self._require_open()
        ser.write(bytes([cmd, value]))
        ser.flush()
    ...
    def _read_exact(self, n: int, timeout_s: float = 2.0) -> bytes:
        ...
        while len(buf) < n and time.time() < deadline:
            ...
            if chunk:
                ...
                time.sleep(0.001)
        if len(buf) != n:
            raise RuntimeError(f"Timed out reading {n} bytes, got {len(buf)}")
        return bytes(buf)
    ...
    def _read_string(self, timeout_s: float = 2.0) -> str:
        """Read a length-prefixed string (length byte + data)"""
    ...
    def _read_response_type(self, timeout_s: float = 2.0) -> int:
        """Read a response type byte"""
        return self._read_byte(timeout_s)

    def _parse_wifi_entry(self, data: bytes) -> dict:
        """Parse a WiFi network entry from binary data"""
        if len(data) < 3:
            return {}
        rssi = data[0] if data[0] < 128 else data[0] - 256  # signed byte
        security = data[1]
        ssid_len = data[2]
    ...
    def _parse_log_entry(self, data: bytes) -> tuple[str, int] | None:
        """Parse a log file entry from binary data"""
        if len(data) < 5:
            return None
        size = data[0] | (data[1] << 8) | (data[2] << 16) | (data[3] << 24)
        path_len = data[4]
        if len(data) < 5 + path_len:
            return None
        path = data[5:5+path_len].decode('utf-8', errors='replace')
        return path, size
